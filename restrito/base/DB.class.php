...
class DB
{
    ...
    public function log_db_errors( $error, $query )
    {
        ...
        $headers .= 'From: Sistema Pesquisa - Prefeitura de Lucas do Rio Verde <site@lucasdorioverde.mt.gov.br>' . "\r\n";
        
        $message  = '<p>Sistema Pesquisa:</p>';
        $message .= '<p>Error at '. date('Y-m-d H:i:s').':</p>';
        $message .= '<p>Query: '. htmlentities( $query ).'<br />';
        $message .= 'Error: ' . $error;
        $message .= '</p>';
        
        mail( SEND_ERRORS_TO, 'Database Error Sistema Pesquisa', $message, $headers);
     ...
     *
     */
    public function insert( $table, $variables = array(), $id = null )
    {
        ...
        if( $id && $variables[$id] ){
        	$this->update( $table, $variables, array( $id => $variables[$id] ), 1 );
        	return true;
        }
